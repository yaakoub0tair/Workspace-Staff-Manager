...
function isWorkerEligible(worker, zoneId) {
    ...
    const isRestrictedZone = ['zone-reception', 'zone-servers', 'zone-security'].includes(zoneId);
    if (isRestrictedZone) {
        return false;
    }
...
function addWorkerToZone(workerData, zoneElement) {
    ...
    const avatarHtml = workerData.photo ? `<div class="employee-avatar" style="background-image: url('${workerData.photo}')"></div>` : `<div class="employee-avatar">${initial}</div>`;
    employeeCard.innerHTML = `<div class="employee-info">${avatarHtml}<div class="employee-details"><h4>${workerData.fullName}</h4><p>${workerData.role}</p></div></div><button class="remove-btn">×</button>`;
    employeeCard.querySelector('.remove-btn').addEventListener('click', handleRemoveWorker);
    staffContainer.appendChild(employeeCard);
    updateZoneState(zoneElement);
}

const selectionModal = document.getElementById('worker-selection-modal');
const selectionList = document.getElementById('worker-selection-list');
const selectionTitle = document.getElementById('selection-modal-title');
...
function openWorkerSelectionModal(eligibleWorkers, zoneElement) {
    selectionTitle.textContent = `Add Worker to ${zoneElement.querySelector('.zone-name').textContent}`;
    selectionList.innerHTML = '';
    const noEligible = eligibleWorkers.length === 0;
    noWorkersMessage.classList.toggle('hidden', !noEligible);
    selectionList.classList.toggle('hidden', noEligible);
    ...
    if (!noEligible) {
        eligibleWorkers.forEach(workerCard => {
            ...
            const initial = workerData.fullName.charAt(0).toUpperCase();
            const avatarHtml = workerData.photo ? `<div class="employee-avatar" style="background-image: url('${workerData.photo}')"></div>` : `<div class="employee-avatar">${initial}</div>`;
            listItem.innerHTML = `${avatarHtml}<div><h4>${workerData.fullName}</h4><p>${workerData.role}</p></div>`;
...
function handleAddWorker(event) {
    ...
    if (currentCount >= config.capacity) return alert(`Capacity full for this zone.`);
    const unassignedWorkers = Array.from(document.querySelectorAll('#list-workers .staff-card'));
    const eligibleWorkers = unassignedWorkers.filter(card => isWorkerEligible(JSON.parse(card.dataset.employee), zoneElement.id));
    openWorkerSelectionModal(eligibleWorkers, zoneElement);
}

export function initializeZones() {
    document.querySelectorAll('.zone').forEach(zone => {
        zone.querySelector('.btn-add-zone').addEventListener('click', handleAddWorker);
        updateZoneState(zone);
    });
    if (selectionModal) {
        selectionModal.querySelector('.btn-close-selection').addEventListener('click', () => selectionModal.classList.add('hidden'));
        selectionModal.addEventListener('click', (event) => {
            if (event.target === selectionModal) selectionModal.classList.add('hidden');
        });
    }
}